...
export const AuthProvider: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    ...
    if (res?.err === 0 && res.accessToken) {
      ...
      try {
        ...
        localStorage.setItem('user', JSON.stringify({ name: `${u.firstName || ''} ${u.lastName || ''}`.trim(), avatar: u.avatar || null }));
  ...
  return (
    <AuthContext.Provider value={{ user, token, login, register, forgotPassword, logout, setToken }}>
      {children}
